...
void FootSwingTrajectory<T>::computeSwingTrajectoryBezier(T phase, T swingTime) {
  ...
  if(phase < T(0.5)) {
    ...
    zp = Interpolate::cubicBezier<T>(_p0[2] + _height, _p0[2], phase * 2 - 1);
    zv = Interpolate::cubicBezierFirstDerivative<T>(_p0[2] + _height, _p0[2], phase * 2 - 1) * 2 / swingTime;
    za = Interpolate::cubicBezierSecondDerivative<T>(_p0[2] + _height, _p0[2], phase * 2 - 1) * 4 / (swingTime * swingTime);
